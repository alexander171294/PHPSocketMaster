...
class DualSocketBridge extends SocketBridge implements iDualSocketBridge
{
	...
	public function onReceiveMessage($message)
	{
		...
		if($this->SendHandshake == true)
		{
			if($message !== null)
			{
				if($this->nsocket == true)
				{
					parent::ValidateObj(array($this->SocketEventReceptor, 'onReceiveMessage'), array($message));	
				} else parent::ValidateObj(array($this->SocketEventReceptor, 'onReceiveMessage'), array($this->unMask($message)));
			} else $this->onDisconnect();
		} else {
			// parsear cabeceras
			$h = $this->parseHeaders($message);
			if(isset($h['Sec-WebSocket-Key']))
			{
				// enviar handshake
	...
	public function send($message, $readControl = false)
	{
		if($this->nsocket == true)
		{
			$this->sendUnmasked($message, $readControl);
		} else parent::send($this->Mask($message), $readControl);
	}
